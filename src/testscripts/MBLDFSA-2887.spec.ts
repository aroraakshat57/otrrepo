...
test.beforeAll(async ( {browser }) => {
 ...
 for (const record of records) {
    test(`Verify whether the vehicle name is proper against each baumster number using Baumster : ${record.Baumster}`, async ({ browser },testInfo) => {
      //Create New Customer
      //console.log("inside testcase "+browser.contexts().length);
      ...
      expectedresult = vehiclename;
      actualresult = otrSteps.vehiclename;
    console.log("expected result: "+vehiclename);
